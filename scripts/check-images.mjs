...
(async function main(){
  ...
  let page = 1; let pageSize = 200; let totalFetched = 0; let total = null;
  while (true) {
    console.log(`Fetching page ${page}`);
    let body;
    try { body = await fetchPage(page, pageSize); } catch (err) { console.error('Fetch error', err); process.exit(1); }
    const items = Array.isArray(body.items) ? body.items : [];
    if (total === null && typeof body.total === 'number') total = body.total;
