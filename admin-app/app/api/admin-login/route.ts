...
export async function POST(req: Request) {
  ...
  async function getAndIncr(key: string) {
    // Lazy-load ioredis at runtime only if REDIS_URL is configured. This avoids
    // a hard dependency at build-time which can cause Next to fail when the
    // optional package (or its sub-deps) are missing in dev environments.
    if (!redisClient && !redisTried && process.env.REDIS_URL) {
      redisTried = true;
  try {
  // Import via a computed string to avoid Next/Webpack statically
  // resolving the `ioredis` package (and its optional sub-deps)
  // at build time. This keeps ioredis optional at build and only
  // attempts to load it at runtime when `REDIS_URL` is set.
  const mod = await import('i' + 'oredis');
        const RedisCls = mod?.default ?? mod;
        try {
          // Prefer `new` if constructable
          // @ts-ignore - runtime check: some package builds export a callable factory
          redisClient = new RedisCls(process.env.REDIS_URL, { enableOfflineQueue: false });
        } catch (err) {
          try {
            // Some bundlings expose a factory function
            // @ts-ignore
            redisClient = RedisCls(process.env.REDIS_URL, { enableOfflineQueue: false });
          } catch (err2) {
            redisClient = null;
          }
        }
      } catch (e) {
        // couldn't load ioredis at runtime; fall back to in-memory store
        redisClient = null;
      }
    }
    if (redisClient) {
  ...
  // Default development password is 'ayperi2017' unless overridden by ADMIN_PASSWORD in env
  const legacy = process.env.ADMIN_PASSWORD || 'ayperi2017';
