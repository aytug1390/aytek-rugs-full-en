...
function normalizeId(v) {
  ...
  const m = s.match(/^([0-9]+)(?:\.0+)?$/);
  return m ? m[1] : s;
}

function buildFilterForPid(pid) {
  const npid = normalizeId(pid);
  const num = Number(npid);
  const or = [{ product_id: String(npid) }];
...
async function main() {
  ...
  const pids = new Set(rows.map(r => normalizeId(r.product_id || "")).filter(Boolean));
  ...
  const pid = normalizeId(row.product_id || "");
    if (!pid) continue;
  ...
  const images = ids.map((id, idx) => ({ url: toDriveUrl(id), alt: `Product ${pid} Image ${idx + 1}`, isPrimary: idx === 0 }));
