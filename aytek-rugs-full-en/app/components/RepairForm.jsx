...
export default function RepairForm() {
  ...
  const validate = () => {
    ...
    if (!form.description.trim()) return "Please describe the repair needed.";
  ...
  const onSubmit = async (e) => {
    ...
    setSending(true);
    try {
      const res = await fetch("/api/service/repair", {
        method: "POST",
        body: fd,
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Failed");
      setMsg({ type: "ok", text: "Thanks! Your repair request has been sent." });
      setForm({
  ...
  return (
    <section id="repair-form" className="max-w-3xl mx-auto bg-white rounded-2xl shadow p-6 mt-12">
      <h2 className="text-2xl font-bold mb-4">Request a Rug Repair & Restoration</h2>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="repair-description" className="block text-sm mb-1">Repair Details *</label>
          <textarea
            ...
            placeholder="Describe the damage (edges, holes, reweaving, etc.)"
            required
          />
        </div>
        ...
        >
          {sending ? "Sending..." : "Send Repair Request"}
        </button>
