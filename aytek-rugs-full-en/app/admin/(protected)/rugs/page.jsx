...
async function fetchJSON(url, opts = {}) {
  ...
  const fetchUrl = isServer && !/^https?:\/\//i.test(url) ? (process.env.NEXT_PUBLIC_API_BASE || "http://127.0.0.1:5000") + url : url;
...
export default function AdminRugsPage() {
  ...
  async function load() {
    ...
    setErr("");
    try {
  const data = await fetchJSON(`/api/admin-api/products?${qs}`);
  ...
  // We intentionally only re-run when `qs` changes.
  ...
  const res = await fetchJSON(`/api/admin-api/products/${row._id}`, {
        method: "PATCH",
  ...
  async function remove(row) {
    ...
    try {
  await fetchJSON(`/api/admin-api/products/${row._id}`, { method: "DELETE" });
  ...
  const created = await fetchJSON(`/api/admin-api/products`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div>
          <label htmlFor="filter-product-id" className="block text-sm font-medium">Filter by Product ID</label>
          <input
        ...
        <div>
          <label htmlFor="filter-q" className="block text-sm font-medium">Search (title/sku)</label>
          <input
            id="filter-q"
            name="q"
            value={q}
      ...
      <details className="rounded-lg border bg-white">
        ...
        <form onSubmit={createOne} className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <label className="text-sm" htmlFor="admin-availability-select">
            <span className="block mb-1">Availability</span>
            <select
              id="admin-availability-select"
