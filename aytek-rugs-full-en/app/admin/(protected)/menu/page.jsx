...
export default function MenuAdminPage() {
  ...
  <button type="button" onClick={addMenu} className="bg-green-600 text-white px-4 py-2 rounded">Add</button>
      </div>
        ...
        {menus?.map(m => (
          <li key={m._id} className="flex items-center justify-between bg-white border rounded p-3">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => toggleActive(m)} className="px-3 py-1 text-xs rounded bg-indigo-600 text-white">
                {m.active ? 'Disable' : 'Enable'}
              </button>
              <button type="button" onClick={() => removeMenu(m._id)} className="px-3 py-1 text-xs rounded bg-red-600 text-white">Delete</button>
            </div>
          </li>
        ))}
      </ul>
