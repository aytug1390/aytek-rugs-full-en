...
export default function QuoteCTA({ label = "Free Rug Repair Quote (2 min)", variant = "A" }) {
  ...
  <div className="fixed inset-0 z-70 flex items-center justify-center">
        ...
        <span>{label}</span>
                  <button type="button" onClick={() => setOpen(false)} className="text-2xl leading-none">×</button>
                </div>
                  ...
                  <div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <Field label="Service Type" required>
                          <select id="quote-service" value={form.service} onChange={e=>setForm(f=>({...f,service:e.target.value}))}
                            className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500">
                ...
                <div className="mt-6 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    {step > 1 && (
                      <button type="button" onClick={()=>setStep(s=>s-1)} className="rounded-lg px-4 py-2 text-sm font-semibold border">
                        Back
                      </button>
                    )}
                    {step < 2 && (
              ...
              <div className="text-center py-10">
                ...
                <button type="button" onClick={()=> setOpen(false)} className="mt-6 rounded-lg bg-teal-600 px-5 py-2 text-sm font-semibold text-white">
                  Close
                </button>
              </div>
            )}
