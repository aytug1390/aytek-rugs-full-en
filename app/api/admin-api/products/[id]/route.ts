...
export async function GET(_req: Request, ctx: { params: Promise<Params> }) {
  ...
  if (!r.ok) {
    ...
    if (r.status === 404) {
      ...
      try {
        const listRes = await fetch(`${UPSTREAM}/products?product_id=${encodeURIComponent(id)}&limit=1`, {
          headers: { Accept: "application/json" },
          cache: "no-store",
        });
        ...
        try {
          ...
          for (let p = 1; p <= MAX_PAGES; p++) {
            const pageRes = await fetch(`${UPSTREAM}/products?page=${p}&limit=${PAGE_SIZE}`, { headers: { Accept: 'application/json' }, cache: 'no-store' });
            ...
            if (MONGO_URI) {
              ...
              try {
                ...
                if (doc && doc.product_id) {
                  ...
                  const listRes2 = await fetch(`${UPSTREAM}/products?product_id=${encodeURIComponent(pid)}&limit=1`, { headers: { Accept: 'application/json' }, cache: 'no-store' });
                  if (listRes2.ok) {
